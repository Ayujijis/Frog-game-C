...
void storkmove(Config config, char **board, int frog[2], stork *thestork) {
    static char previouschar = ' ';  // Tracks the character where the stork is
    
    if (thestork[0].movecheck >= 20) {
        // Restore the previous character where the stork was
        board[thestork[0].y][thestork[0].x] = previouschar;
        ...
        // Store the character
        previouschar = board[thestork[0].y][thestork[0].x];
...
void levelstart(Config config, char **board, rock *rocks, int *frog, car *cars, stork *thestork){
...
int wincheck (Config config, char **board, rock *rocks, int *frog, time_t start_time, car *cars, stork *thestork){

if (frog[1]==(TOPOFBOARD)){
levelstart(config, board, rocks, frog, cars, thestork);
...
while (exit==0)
{
    ...
    score+=wincheck(config, board, rocks, frog, start_time, cars, thestork);
